...
### Deploy steps
1. Build frontend: `cd apps/web && bun run build`
2. Start production server: `DATABASE_URL=./prod.db bun run apps/server/src/index.ts`
3. Confirm API responds: `curl -s http://localhost:3000/api/dashboard`
...
- Core user flow walkthrough passes (as defined in PRD user stories)
